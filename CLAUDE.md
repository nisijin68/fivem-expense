...
## ✅ 2026-06-30 移行案内バナーにログイン方法を追記

### 🎯 内容
ブロック時の案内バナーに、新サイトのログイン方法を追加表示（メール案内文と同内容）。
- ログインメールアドレス：旧サイト登録メールアドレスと同じ
- 初期パスワード：メールアドレスの「@」より前の部分（例: tanaka@example.com → tanaka）
- ログイン後は「アカウント設定」から必ずパスワード変更するよう案内
- ログインできない／パスワードを忘れた場合は新サイトの「パスワードを忘れた場合」からリセット、解決しない場合は管理者へ連絡

### 📋 修正ファイル
- `client/src/components/ExpenseForm.tsx`: 案内バナーにログイン方法セクションを追加
